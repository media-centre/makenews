import { addRssUrl, addFacebookPage, addTwitterHandle, RSS_ADD_URL_STATUS } from "./../../src/js/config/actions/AddUrlActions";
...
describe("AddUrl Actions", () => {
    ...
    describe("AddFacebookPage", () => {
        ...
        it("should return successful message", (done) => {
            ...
            const action = [{ "type": FACEBOOK_ADD_PAGE, "sources": [{ "id": 1, "name": "page name" }] }];
            const store = mockStore([], action, done);
            store.dispatch(addFacebookPage(pageUrl));
        });

        it("should should show toast message if an error occurs while adding the page", async() => {
            const pageUrl = "test";
            ajaxClientInstance = AjaxClient.instance("/configure-facebook-page");
            sandbox.stub(AjaxClient, "instance").returns(ajaxClientInstance);
    ...
    describe("AddTwitterHandle", () => {
        ...
        it("should return successful message", (done) => {
            ...
            const action = [{ "type": TWITTER_ADD_SOURCE, "sources": [{ "id": 1, "name": "handle name" }] }];
            const store = mockStore([], action, done);
            store.dispatch(addTwitterHandle(twitterHandle));
        });

        it("should should show toast message if an error occurs while adding the handle", async () => {
            ...
            await (addTwitterHandle(twitterHandle)());
            toastMock.verify();
        });
    });
});
