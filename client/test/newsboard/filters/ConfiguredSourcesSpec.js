...
describe("ConfiguredSources", () => {
    ...
    it("should call renderSources with current sourceType and searchKey", () => {
        ...
        sourceFilter.render(<ConfiguredSources currentTab={"twitter"} searchKeyword={"the"} renderSources={renderSourceFun} />);
    });

    it("should have a root div with class list-sources-block ", () => {
        const wrapper = shallow(<ConfiguredSources currentTab="web" searchKeyword={""} renderSources={anonymousFun} />);
    ...
    it("should have a h4 with the heading of Twitter if currentTab is twitter", () => {
        const wrapper = shallow(<ConfiguredSources currentTab="twitter" searchKeyword={""} renderSources={anonymousFun} />);
        const listSourceGroupHeading = wrapper.find(".list-sources__group__heading");
    ...
    it("should have two h4 with the heading of Facebook Pages, Facebook Groups if currentTab is not twitter or web", () => {
        const wrapper = shallow(<ConfiguredSources currentTab="pages" searchKeyword={""} renderSources={anonymousFun} />);
