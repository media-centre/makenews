...
curl -g -X PUT http://"$1":"$2"@localhost:5984/$3 2>/dev/null
if [ "$?" -ne 0 ]
then
...
curl -g -X PUT http://"$1":"$2"@localhost:5984/$3/_security -d '{ "admins": { "names": [], "roles": [] }, "members": { "names": ["'$1'"], "roles": [] } }' 2>/dev/null
if [ "$?" -ne 0 ]
then
...
curl -X PUT http://"$1":"$2"@localhost:5984/_users
if [ "$?" -ne 0 ]
then
...
curl -X PUT http://"$1":"$2"@localhost:5984/_replicator
if [ "$?" -ne 0 ]
then
...
curl -X PUT http://"$1":"$2"@localhost:5984/_global_changes
if [ "$?" -ne 0 ]
then
