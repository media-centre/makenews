...
describe("FetchFeedsFromAllSources", () => {
    ...
    describe("FetchFeeds", () => {
        ...
        it("should send error response if there is an invalid data in the request body", async() => {
            ...
            fetchFeedsRequest = new FetchFeedsFromAllSources(requestBody, response);
            const fetchFeedsFromAllSourcesMock = sandbox.mock(fetchFeedsRequest).expects("fetchFeedsFromAllSources");
    ...
    describe("fetchFeedsFromAllSources", () => {
        it("should fetch the feeds when since is not present in source url", async() => {
            const urlDocuments = [{ "sourceType": "web", "_id": "http://toi.timesofindia.indiatimes.com/rssfeedstopstories.cms" }];
            const expectedFeeds = { "docs": [{ "_id": 1, "title": "Something" }, { "_id": 2, "title": "Something1" }], "paging": {} };
            const fetchFeedsFromSourceMock = sandbox.mock(fetchFeedsRequest).expects("fetchFeedsFromSource");
            fetchFeedsFromSourceMock.returns(Promise.resolve(expectedFeeds));
            const getUrlDocumentsMock = sandbox.mock(fetchFeedsRequest).expects("_getUrlDocuments");
            getUrlDocumentsMock.returns(Promise.resolve(urlDocuments));
            const updateTimeMock = sandbox.mock(fetchFeedsRequest).expects("updateUrlTimeStamp");
        ...
        it("should get the feeds only when the latestTimeStamp interval is more", async() => {
            ...
            const getUrlDocumentsMock = sandbox.mock(fetchFeedsRequest).expects("_getUrlDocuments");
            getUrlDocumentsMock.returns(Promise.resolve(urlDocuments));
            const updateTimeStamp = sandbox.mock(fetchFeedsRequest).expects("updateUrlTimeStamp");
    ...
    describe("fetchFeedsFromSource", () => {
        ...
        describe("web", () => {
            it("should return feeds for valid web url", async() => {
                ...
                const rssRequestHandlerMock = sandbox.mock(RssRequestHandler).expects("instance");
                rssRequestHandlerMock.returns(rssRequestHandlerInstance);
                const fetchRssFeedRequestMock = sandbox.mock(rssRequestHandlerInstance).expects("fetchBatchRssFeedsRequest");
        ...
        describe("facebook", () => {
            it("should return feeds for valid facebook page url", async() => {
                ...
                const getFacebookTokenMock = sandbox.mock(fetchFeedsRequest).expects("_getFacebookAccessToken");
                getFacebookTokenMock.returns("facebook_token");
                const facebookRequestHandlerInstance = new FacebookRequestHandler("test_token");
                const facebookRequestHandlerMock = sandbox.mock(FacebookRequestHandler).expects("instance");
                facebookRequestHandlerMock.withArgs("facebook_token").returns(facebookRequestHandlerInstance);
                const fetchFacebookFeedRequestMock = sandbox.mock(facebookRequestHandlerInstance).expects("fetchFeeds")
            ...
            it("should return empty array for invalid facebook page url", async() => {
                ...
                const facebookRequestHandlerMock = sandbox.mock(FacebookRequestHandler).expects("instance");
                facebookRequestHandlerMock.withArgs("facebook_token").returns(facebookRequestHandlerInstance);
                const fetchFacebookFeedRequestMock = sandbox.mock(facebookRequestHandlerInstance)
            ...
            it("should return feeds for valid facebook group url", async() => {
                ...
                const facebookRequestHandlerMock = sandbox.mock(FacebookRequestHandler).expects("instance");
                facebookRequestHandlerMock.withArgs("facebook_token").returns(facebookRequestHandlerInstance);
                const fetchFacebookFeedRequestMock = sandbox.mock(facebookRequestHandlerInstance).expects("fetchFeeds").withArgs(urlDocument._id, "feed");
        ...
        describe("twitter", () => {
            it("should return feeds for valid twitter url", async() => {
                ...
                const twitterRequestHandlerMock = sandbox.mock(TwitterRequestHandler).expects("instance");
                twitterRequestHandlerMock.returns(twitterRequestHandler);
                const fetchTwitterFeedRequestMock = sandbox.mock(twitterRequestHandler).expects("fetchTweetsRequest")
    ...
    describe("saveFeedDocumentsToDb", () => {
        it("should throw failed to store in db when there is an error save bulk docs", async() => {
            ...
            const rss = { "sourceType": "rss", "_id": "http://toi.timesofindia.indiatimes.com/rssfeedstopstories.cms" };
            const facebook = { "sourceType": "twitter", "_id": "@TheHindu" };
            const twitter = { "sourceType": "facebook", "_id": "http://www.facebook.com/thehindu" };
            const requestData = {
                "body": {
                    "data": [
                        rss,
            ...
            const dbName = "dbName";
            const feeds = { "items": ["feed items"] };
            const couchClientMock = new CouchClient("accessToken");
            const fetchFeedsFromAllSources = new FetchFeedsFromAllSources(requestData, response);
            try {
        ...
        it("should return success message when there is no error", async() => {
            ...
            try {
                ...
                sinon.mock(CryptUtil).expects("dbNameHash").withArgs(response.userCtx.name).returns(dbName);
                const res = await fetchFeedsFromAllSources.saveFeedDocumentsToDb(feeds);
    ...
    describe("GetUrlDocs", () => {
        it("should get all url docs", async() => {
            ...
            const couchClientInstance = new CouchClient();
            const findDocsMock = sandbox.mock(couchClientInstance).expects("findDocuments")
                .withArgs(selector).returns({ "docs": [rss, facebook, twitter] });

            const resp = await fetchFeedsRequest._getUrlDocuments(couchClientInstance);
            findDocsMock.verify();
        ...
        it("should get the url docs recursively, if there are more than 25 url docs", async() => {
            ...
            const couchClientInstance = new CouchClient();
            const findDocsStub = sandbox.stub(couchClientInstance, "findDocuments");
    ...
    describe("GetFacebookAccessToken", () => {
        ...
        beforeEach("GetFacebookAcessToken", () => {
            sandbox = sinon.sandbox.create();
            const applicationConfigInstance = new ApplicationConfig();
        ...
        it("should get the facebook access token", async() => {
            ...
            const findDocumentsMock = sandbox.mock(adminDbClient).expects("findDocuments");
            ...
            const request = { "cookies": { "AuthSession": "test_token" }, "body": { "data": [] } };
            response = {};
            const fetchFeedsFromAllSources = new FetchFeedsFromAllSources(request, response);
            const token = await fetchFeedsFromAllSources._getFacebookAccessToken();
